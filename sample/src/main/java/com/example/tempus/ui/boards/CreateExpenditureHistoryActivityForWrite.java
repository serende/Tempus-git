        ...
        switch (requestCode) {
            case REQUEST_TAKE_PHOTO:
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    Toast.makeText(CreateExpenditureHistoryActivityForWrite.this, "사진찍기를 취소하였습니다.", Toast.LENGTH_SHORT).show();
                }
                break;
        ...
        switch (requestCode) {
            case MY_PERMISSION_CAMERA:
                for (int i = 0; i < grantResults.length; i++) {
                    ...
                    if (grantResults[i] < 0) {
                        Toast.makeText(CreateExpenditureHistoryActivityForWrite.this, "해당 권한을 활성화 하셔야 합니다.", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
                break;
